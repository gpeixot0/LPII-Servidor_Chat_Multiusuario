#  Servidor de Chat Multiusuário Concorrente (TCP em C)
...
A biblioteca usa **mutexes internos** para garantir exclusão mútua durante a escrita no log.
...
1. **Comunicação TCP padrão:** mostra as chamadas `socket()`, `bind()`, `listen()`, `accept()` e `connect()` entre cliente e servidor.  
2. **Conexão inicial e entrada no chat:** explica o momento em que o cliente se conecta, envia seu nome e o servidor anuncia sua entrada aos demais usuários.  
3. **Envio e recebimento de mensagens:** ilustra o fluxo de mensagens entre diferentes clientes, passando pela fila de mensagens e o *broadcaster*.  
4. **Saída do cliente e encerramento do servidor:** descreve o encerramento de conexões e o tratamento de sinais (`SIGINT` e `/quit`).
...
Para compilar o projeto completo, execute:
